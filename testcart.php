...
body {
  ...
  position: absolute;
	...
	<body>
		<script>
			window.onload = function() {	//把“匿名函数”赋值给window的onload事件属性
				var aData = [{
				...
				var oBox = document.getElementById("box");	//根據ID取得網頁上的元素
				var oCar = document.getElementById("car");
				var oUl = document.getElementsByTagName("ul")[0];

				for (var i = 0; i < aData.length; i++) {	//跑data的長度
					var oLi = document.createElement("li");	//依指定的標籤名稱（tagName）建立 HTML 元素
					var data = aData[i];
					...
					oLi.innerHTML += '<p class="pro_rank">' + data["proComm"] + ' 庫存</p>';
				...
				for (var i = 0; i < aBtn.length; i++) {
					...
					aBtn[i].onclick = function() {
						...
						check.onclick = function() {	// console.log(check.className);
						...
						for (var k = 0; k < i_btn.length; k++) {
							i_btn[k].onclick = function() {
								bt = this;//获取小计节点								
								at = this.parentElement.parentElement.nextElementSibling;//获取单价节点								
								pt = this.parentElement.parentElement.previousElementSibling;//获取数量值								
								node = bt.parentNode.childNodes[1];
								console.log(node);
								num = node.innerText;
								num = parseInt(num);
								num++;
								node.innerText = num;//获取单价								
								price = pt.innerText;
								price = price.substring(0, price.length - 1);//计算小计值
								at.innerText = price * num + "元";//计算总计值								
								getAmount();
							}
						}
						//获取所有的数量减号按钮
						...
						for (k = 0; k < i_btn.length; k++) {
							d_btn[k].onclick = function() {
								bt = this;	//获取小计节点
								at = this.parentElement.parentElement.nextElementSibling;	//获取单价节点
								pt = this.parentElement.parentElement.previousElementSibling;	//获取c_num节点
								node = bt.parentNode.childNodes[1];
								num = node.innerText;
								num = parseInt(num);
								if (num > 1) {
			...
			var index = false;
			function checkAll() {
				var choose = document.getElementById("car").getElementsByTagName("i");	// console.log(choose);
				if (choose.length != 1) {
			...
			//进行价格合计
			function getAmount() {		// console.log(ys);
				ns = document.getElementsByClassName("i_acity");
				console.log(ns);
				sum = 0;		//选中框
				document.getElementById("price_num").innerText = sum;
				for (y = 0; y < ns.length; y++) {	//小计
		...
		<body>
			...
			<div id="box">
				<h2 id="h5" class="box_head"><span>買購物車商品的人還買了</span></h2>
				<ul>
				</ul>
			</div>
		</body>
